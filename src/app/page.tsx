...
export default function SocialFeedPage() {
  ...
  const handleAddPost = useCallback(async (content: string) => {
    ...
    try {
      await addDoc(collection(db, 'posts'), {
        ...
        authorPhotoURL: user.photoURL || `https://placehold.co/40x40.png?text=${user.name.charAt(0)}`,
        content: content,
  ...
  return (
    <AuthGuard>
      {(authLoading || isDataLoading) && !posts.length ? (
        <SocialSkeleton />
      ) : (
        <div className="bg-background min-h-screen">
          <Header />
          <main className="container mx-auto p-4 grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
            <aside className="hidden md:block md:col-span-1">
